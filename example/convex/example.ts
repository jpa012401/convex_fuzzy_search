...
import { exposeApi } from "@elevatech/typesense-search";
...
export const addComment = mutation({
  ...
  handler: async (ctx, args) => {
    return await ctx.runMutation(components.typesenseSearch.lib.add, {
      text: args.text,
...
export const listComments = query({
  ...
  handler: async (ctx, args) => {
    return await ctx.runQuery(components.typesenseSearch.lib.list, {
      targetId: args.targetId,
    });
  },
});
...
export const translateComment = action({
  ...
  handler: async (ctx, args) => {
    return await ctx.runAction(components.typesenseSearch.lib.translate, {
      baseUrl: BASE_URL,
...
// the component's API:
export const { list, add, translate } = exposeApi(components.typesenseSearch, {
