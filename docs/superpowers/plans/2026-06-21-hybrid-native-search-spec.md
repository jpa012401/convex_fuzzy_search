...
- **AND recall edge:** if native's top-K (≤1024) holds too few all-token docs for a very broad multi-word query, the AND page can under-fill. Mitigation: K at the max useful (≤1024), flag `found_approximate`. Acceptable given <1000-doc request ceiling.
